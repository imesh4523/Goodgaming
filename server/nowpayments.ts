...
export async function createNOWPayment(amountUSD: string, currency: string, storage?: IStorage): Promise<NOWPaymentResponse | null> {
  ...
  if (IS_MOCK) {
    ...
    return {
      ...
      price_currency: 'USD',
      ipn_callback_url: `${process.env.REPLIT_DEV_DOMAIN || 'http://localhost:5000'}/api/payments/webhook`,
  ...
  try {
    const response = await fetch(`${NOWPAYMENTS_API_URL}/payment`, {
      ...
      body: JSON.stringify({
        ...
        price_currency: 'USD',
        pay_currency: apiCurrency,
        ipn_callback_url: process.env.REPLIT_DEV_DOMAIN 
          ? `https://${process.env.REPLIT_DEV_DOMAIN}/api/payments/webhook`
