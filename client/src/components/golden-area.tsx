...
export default function GoldenArea(props: GoldenAreaProps) {
  ...
  useEffect(() => {
    if (goldenLiveStats) {
      ...
      // Initialize dynamic values with backend data
      setDynamicTotalPlayers((goldenLiveStats as GoldenLiveStats).totalPlayers);
      // Initialize active players with random value between 1200-18000
      const randomActive = Math.floor(Math.random() * (18000 - 1200 + 1)) + 1200;
      setDynamicActivePlayers(randomActive);
    }
  }, [goldenLiveStats]);

  // Dynamic total players update: increase by 1-4 every 2 seconds
  useEffect(() => {
    if (!stats) return;
    
    const interval = setInterval(() => {
      const increment = Math.floor(Math.random() * 4) + 1; // Random 1-4
      setDynamicTotalPlayers(prev => prev + increment);
