    ...
    onSuccess: (data) => {
      ...
      flipTimeoutRef.current = setTimeout(() => {
        ...
        setGameHistory(prev => [{
          ...
          amount: betAmount,
          won: data.won,
          winAmount: data.won ? usdToGoldCoins(parseFloat(data.winAmount)) : 0,
          timestamp: new Date()
        }, ...prev.slice(0, 9)]);
                    ...
                    >
                      ...
                      <div className="text-right">
                        <div className={`font-bold ${game.won ? 'text-green-400' : 'text-red-400'}`}>
                          {game.won ? '+' : '-'}{formatGoldCoins(game.won && game.winAmount ? game.winAmount : game.amount)}
                        </div>
                        <div className="text-xs text-white/60">
                          Bet: {formatGoldCoins(game.amount)}
                        </div>
                      </div>
                    </div>
                  ))}
